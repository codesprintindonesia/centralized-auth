...
import { UserModel, TokenModel, ConsumerModel, ProviderKeyModel } from '../models/index.model.js';
import { 
  verifyPassword, 
  signData, 
  generateRandomToken, 
...
export const authenticateUser = async (credentials, context) => {
  ...
  try {
    ...
    if (!consumer || !consumer.is_active) {
      logger.warn(`Authentication attempt with invalid consumer: ${consumerId}`);
      return { 
        success: false, 
    ...
    if (consumer.allowed_ips && consumer.allowed_ips.length > 0) {
      ...
      if (!isAllowed) {
        logger.warn(`Authentication attempt from unauthorized IP: ${ipAddress}`);
        return { 
          success: false, 
    ...
    if (!passwordMatch) {
      ...
      if (user.failed_attempts + 1 >= 5) {
        ...
        logger.warn(`Account locked after too many failed attempts: ${username}`);
        return { 
          success: false, 
          code: 'ACCOUNT_LOCKED',
...
export const verifyToken = async (token, consumerId) => {
  try {
    ...
    if (tokenRecord.consumer_id !== consumerId) {
      logger.warn(`Token consumer mismatch: expected=${consumerId}, actual=${tokenRecord.consumer_id}`);
      return { 
        success: false, 
...
export const revokeToken = async (tokenId, context) => {
  ...
  try {
    ...
    logger.info(`Token revoked: ${tokenId}, user: ${userId}, reason: ${reason}`);
...
export const revokeAllTokensForUser = async (userId, context) => {
  ...
  try {
    ...
    logger.info(`All tokens revoked for user: ${userId}, count: ${count}, reason: ${reason}`);
